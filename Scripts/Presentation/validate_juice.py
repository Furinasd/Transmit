...
class JuiceRun(CandidateCaptureRun):
 def __init__(self):
  ...
  reset_source='Pacing_LearnSourceA' if 'Pacing_LearnSourceA' in self.a else 'Learn_Source'
  for repeat in range(3):
   if reset_source=='Pacing_LearnSourceA':self.walk((-8500,0))
   self.add('capture for active reset '+str(repeat),lambda:self.capture_cancel(reset_source,False))
 ...
 def emit(self,event,**kwargs):
  ...
  views={'capture practice A':'12-practice-bank','capture practice B':'13-north-operation',
         'walk (5350, -3500)':'14-service-guidance','reclaim crossing resource':'15-reclaim',
         'walk (7000, -600)':'16-arena-approach'}
  if event=='step' and kwargs.get('name') in views:
   self.frames.append((self.now()+.05,views[kwargs['name']]))
 ...
 def finish(self,ok,reason):
  ...
  unreal.EditorPythonScripting.set_keep_python_script_alive(globals().get('TRANSMIT_KEEP_EDITOR_OPEN',False))
